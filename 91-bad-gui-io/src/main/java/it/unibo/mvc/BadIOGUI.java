...
public class BadIOGUI {
    ...
    public BadIOGUI() {
        ...
        final JPanel ncanvas = new JPanel();
        ncanvas.setLayout(new BoxLayout(ncanvas, BoxLayout.LINE_AXIS));
        canvas.add(ncanvas, BorderLayout.CENTER);
        ncanvas.add(write);
            ...
            public void actionPerformed(final ActionEvent e) {
                try {
                    ...
                    for (final String line : lines) {
                        System.out.println(line); //NOPMD: is only an exercise
                    }
                } catch (IOException e1) {
                    JOptionPane.showMessageDialog(frame, e1, "Error", JOptionPane.ERROR_MESSAGE);
                    e1.printStackTrace(); //NOPMD: is only an exercise
                }
            }
        });
    }
